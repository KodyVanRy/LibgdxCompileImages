...
def get_image_assets(files, root):
    ...
    for _file in files:
        ...
        if _file.split(".")[0] + ".fnt" not in files \
                ...
                and "big_picture_a_" not in _file\
                and "big_picture_b_" not in _file:
...
class BigPicture(pygame.Surface):
        ...
        for y in range(0, self.get_height()):
            for x in range(0, self.get_width()):
                ...
                if draw:
                    ...
                    static_out.append("public static final int " + _image.get_static_name() + " = " + str(len(static_out)) + ";")
    ...
    def add_image_faster(self, _image, big_picture=0):
        print "add_image"
        y = 0
        while y < self.get_height() - _image.get_height():
            x = 0
            while x < self.get_width() - _image.get_width():
                draw = True
                for surface in self.surfaces:
                    if surface.rect.colliderect(
                            pygame.Rect(x - 2, y, _image.get_width(), _image.get_height())):
                        x += 2
                        draw = False
                    if surface.rect.colliderect(
                            pygame.Rect(x, y - 2, _image.get_width(), _image.get_height())):
                        y += 2
                        draw = False
                    if surface.rect.colliderect(
                            pygame.Rect(x, y, _image.get_width() + 2, _image.get_height() + 2)):
                        if (self.get_width() - x) < _image.get_width():
...
class Controls(pygame.Surface):
    def __init__(self, on_click_listener):
        pygame.Surface.__init__(self, (1024, 50))
    ...
    def init_buttons(self):
        ...
        self.buttons.append(Button(60, 0, "LEFT"))
        self.buttons.append(Button(120, 0, "RIGHT"))
        self.buttons.append(Button(180, 0, "H_IN"))
        self.buttons.append(Button(300, 0, "BOTTOM"))
        self.buttons.append(Button(360, 0, "TOP"))
        self.buttons.append(Button(420, 0, "V_IN"))
        self.buttons.append(Button(540, 0, "DONE", color=(0, 255, 0)))
...
def get_1024_a(_images):
    r_images = list()
    big_picture = BigPicture(2048, 2048)
    for _image in _images:
        if not big_picture.add_image_faster(_image):
...
def onclick(button):
    ...
    if "ninepatch" in button.title.lower():
    ...
    elif "h_in" in button.title.lower():
        control = CONTROL_H_IN
        ...
        control = CONTROL_TOP
    elif "v_in" in button.title.lower():
        control = CONTROL_V_IN
